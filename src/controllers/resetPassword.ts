...
import { User, Password } from "../services/database/index.js";

const TOKEN_LENGTH = 32;


export async function generatePasswordResetToken(req: Request, res: Response) {
  ...
  await Password.registerResetRequest(email, token, BigInt(expiry));
...
export async function confirmPasswordReset(req: Request, res: Response) {
  ...
  const user = await User.findExistingEmail(result.email);
  if (!user) {
